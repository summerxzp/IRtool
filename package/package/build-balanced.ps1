param(
    [ValidateSet('onefile','onedir','onedir-upx')] [string]$Mode = 'onedir-upx'
)
...
if ($useUpx) {
    ...
    Write-Host "UPX not found. For smaller size, download UPX to $upxDir" -ForegroundColor Yellow
}
...
switch ($Mode) {
    'onefile' {
        $buildArgs += '--onefile'
        Write-Host "Mode: onefile" -ForegroundColor Cyan
    }
    'onedir' {
        $buildArgs += '--onedir'
        Write-Host "Mode: onedir" -ForegroundColor Cyan
    }
    'onedir-upx' {
        $buildArgs += '--onedir'
        if ($useUpx) {
            $buildArgs += @('--upx-dir', $upxDir)
            Write-Host "Mode: onedir + UPX (Recommended)" -ForegroundColor Green
        } else {
...
if ($LASTEXITCODE -eq 0) {
    ...
    switch ($Mode) {
        'onefile' {
            $exePath = Join-Path $here 'dist\sectool.exe'
        ...
        default {
            $dirPath = Join-Path $here 'dist\sectool'
