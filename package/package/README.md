...
| 模式 | 输出文件名 | 大小 | 说明 |
|------|-----------|------|------|
| `onedir-7z` | `sectool-onedir-7z.exe` | ~37MB | **推荐**，自解压运行 |
| `onefile` | `sectool-onefile.exe` | ~58MB | 单文件 |
| `onedir` | `sectool-onedir\` | ~140MB | 目录形式 |
| `onedir-upx` | `sectool-onedir-upx\` | ~50-70MB | UPX压缩目录 |
...
- 查看 `build\sectool-*\warn-sectool-*.txt` 了解详细警告信息
