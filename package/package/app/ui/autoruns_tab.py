...
class AutorunsScanWorker(QThread):
    ...
    def run(self):
        try:
            logger.info(f"[ScanThread] 开始扫描，include_hash={self.include_hash}, verify_sig={self.verify_sig}")
            self.progress.emit("正在扫描自启动项...")
            
            # 检查parser是否存在
            if not self.parser:
                raise ValueError("AutorunsParser未初始化")
            
            logger.debug(f"[ScanThread] parser类型: {type(self.parser)}")
            logger.debug(f"[ScanThread] 调用parser.scan...")
            ...
            logger.info(f"[ScanThread] 扫描完成，共 {len(entries)} 个条目")
            
            # Check if cancelled before emitting finished signal
            if not self._is_cancelled:
                logger.debug(f"[ScanThread] 转换为字典格式...")
                try:
                    entries_dict = [e.to_dict() for e in entries]
                    logger.info(f"[ScanThread] 转换完成，发送 finished 信号")
                    self.finished.emit(entries_dict)
                except Exception as convert_error:
                    logger.error(f"[ScanThread] 转换为字典时出错: {convert_error}")
                    logger.error(traceback.format_exc())
                    self.error.emit(f"数据转换错误: {convert_error}")
            else:
                logger.info(f"[ScanThread] 扫描已取消")
            
        except Exception as e:
            error_msg = str(e)
            error_trace = traceback.format_exc()
            logger.error(f"[ScanThread] 扫描错误: {error_msg}")
            logger.error(f"[ScanThread] 错误堆栈:\n{error_trace}")
            if not self._is_cancelled:
...
class AutorunsTab(QWidget):
    ...
    def _start_scan(self):
        """开始扫描"""
        logger.info("[_start_scan] 用户点击开始扫描")
        
        # 防止重复启动扫描
        if self._is_scanning:
            logger.warning("[_start_scan] 扫描已在进行中，忽略重复请求")
            return
        
        # 检查parser是否可用
        if not self.parser:
            logger.error("[_start_scan] AutorunsParser未初始化")
            QMessageBox.warning(self, "错误", "AutorunsParser未初始化，请检查工具路径")
            return
        ...
        if self.chk_sig.isChecked():
            ...
            if reply == QMessageBox.StandardButton.No:
                logger.info("[_start_scan] 用户取消签名验证扫描")
                return
        
        # 如果当前有正在运行的worker，则取消它
        if self.current_worker and self.current_worker.isRunning():
            logger.debug("[_start_scan] 停止之前的worker")
        ...
        logger.info(f"[_start_scan] 创建Worker: include_hash={self.chk_hash.isChecked()}, verify_sig={self.chk_sig.isChecked()}")
        ...
        try:
            ...
            logger.info("[_start_scan] Worker已启动")
        except Exception as e:
            logger.error(f"[_start_scan] 启动Worker失败: {e}")
            logger.error(traceback.format_exc())
    ...
    def _on_scan_error(self, error_msg):
        """扫描错误处理"""
        logger.error(f"[_on_scan_error] 扫描出错: {error_msg}")
        ...
        # 显示详细错误信息
        error_detail = f"扫描过程中发生错误:\n\n{error_msg}\n\n"
        error_detail += f"日志文件位置:\n{logging.getLogger().handlers[0].baseFilename}"
        QMessageBox.critical(self, "扫描错误", error_detail)
