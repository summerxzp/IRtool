...
class AutorunsParser:
    ...
    def __init__(self, autoruns_path: str = None):
        logger.info(f"[AutorunsParser] 初始化...")
        # 默认从程序目录下的tools文件夹查找
        ...
        logger.info(f"[AutorunsParser] autoruns路径: {self.autoruns_path}")
        self._verify_autoruns()
        logger.info(f"[AutorunsParser] 初始化完成")
    
    def _verify_autoruns(self):
        """验证autoruns是否存在"""
        logger.debug(f"[AutorunsParser] 验证autoruns存在: {self.autoruns_path}")
        if not os.path.exists(self.autoruns_path):
            logger.error(f"[AutorunsParser] autoruns不存在: {self.autoruns_path}")
            raise FileNotFoundError(
                f"autorunsc64.exe not found at {self.autoruns_path}"
            )
        logger.debug(f"[AutorunsParser] autoruns验证通过")
    ...
    def scan(self,
        ...
        """
        logger.info(f"[AutorunsParser.scan] 开始扫描: include_hash={include_hash}, verify_signature={verify_signature}")
        logger.info(f"[AutorunsParser.scan] 类别过滤: {category_filter}")
        ...
        logger.info(f"[AutorunsParser.scan] 执行命令: {' '.join(cmd)}")

        try:
            logger.debug(f"[AutorunsParser.scan] 启动子进程...")
            # 隐藏命令行窗口
            ...
            logger.debug(f"[AutorunsParser.scan] 等待进程完成(超时180秒)...")
            stdout_bytes, stderr_bytes = process.communicate(timeout=180)  # 3分钟超时
            logger.debug(f"[AutorunsParser.scan] 进程完成，返回码: {process.returncode}")

        except subprocess.TimeoutExpired:
            logger.error(f"[AutorunsParser.scan] 扫描超时")
            process.kill()
            process.communicate()
            raise RuntimeError("Autoruns 扫描超时")
        except Exception as e:
            logger.error(f"[AutorunsParser.scan] 执行异常: {e}")
            import traceback
        ...
        if process.returncode != 0:
            ...
            logger.error(f"[AutorunsParser.scan] 进程返回错误码 {process.returncode}: {stderr}")
            raise RuntimeError(f"Autoruns 执行失败: {stderr}")

        logger.debug(f"[AutorunsParser.scan] 输出大小: {len(stdout_bytes)} bytes")

        # === 关键修复：UTF-16LE 解码 ===
        try:
            csv_text = stdout_bytes.decode('utf-16le', errors='ignore')
            logger.debug(f"[AutorunsParser.scan] CSV解码成功，长度: {len(csv_text)} chars")
        except Exception as e:
            logger.error(f"[AutorunsParser.scan] CSV解码失败: {e}")
            raise RuntimeError(f"Autoruns 输出解码失败: {e}")

        logger.debug(f"[AutorunsParser.scan] 开始解析CSV...")
        entries = self._parse_csv(csv_text)
        logger.info(f"[AutorunsParser.scan] CSV解析完成，共 {len(entries)} 个条目")
        ...
        if category_filter:
            ...
            logger.info(f"[AutorunsParser.scan] 类别过滤: {before_filter} -> {len(entries)} 个条目")

        logger.info(f"[AutorunsParser.scan] 扫描完成，返回 {len(entries)} 个条目")
        return entries
